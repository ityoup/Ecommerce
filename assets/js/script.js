...
subwayJson.map( (item, index) => {
    ...
    subwayItem.querySelector('.product--price').innerHTML = `${item.price[0].toLocaleString('pt-MX', {style: 'currency', currency: 'MXN'})} (${item.sizes[0]})`;
    ...
    subwayItem.querySelector('a').addEventListener('click', (event) => {
        ...
        document.querySelector('.productInfo--actualPrice').innerHTML = `${subwayJson[key].price[0].toLocaleString('pt-MX', {style: 'currency', currency: 'MXN'})}`;
...
document.querySelectorAll('.productInfo--size').forEach( (size, sizeIndex) => {
    size.addEventListener('click', () => {
        ...
        if (sizeIndex === 0){
            document.querySelector('.productInfo--actualPrice').innerHTML = `${subwayJson[pizzaKey].price[0].toLocaleString('pt-MX', {style: 'currency', currency: 'MXN'})}`;
        } else {                      
            document.querySelector('.productInfo--actualPrice').innerHTML = `${subwayJson[pizzaKey].price[1].toLocaleString('pt-MX', {style: 'currency', currency: 'MXN'})}`;
        }
    });
});
...
function updateCart() {
    if (cart.length > 0) {
        ...
        for (let i in cart) {
            ...
            cartItem.querySelector('.cart--item-price').innerHTML = `${subwayItem.price[cart[i].size].toLocaleString('pt-mx',{ style:'currency', currency: 'MXN' })}`;
        ...
        document.querySelector('.total span:last-child').innerHTML = `${total.toLocaleString('pt-mx',{ style:'currency', currency: 'MXN' })}`;
